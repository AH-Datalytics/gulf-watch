...
REGISTER DIRECTIVE (user): "lived-in product, not an AI war room viewscreen." Concretely: mixed-case, plain-language labels and headings (e.g. "Wind chances in New Orleans", "Forecast models"); all-caps/monospace ONLY for genuinely tabular figures; normal product chrome (simple header, standard buttons/toggles/layer-switcher in familiar map-app conventions); generous whitespace and line-height; minimal rules/chips/badges; calm neutrals, warning colors reserved for warnings; no HUD/terminal aesthetics anywhere.
